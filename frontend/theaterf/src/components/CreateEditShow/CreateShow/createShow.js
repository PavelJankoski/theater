...
const CreateShow = (props) => {
        ...
        return (
            <div className="checkbox" key={actor.id} actorid={actor.id}>
                <label><input type="checkbox" name={"actor"} value={actor.id}
                              style={{marginRight: '7px'}}/>{actor.name} {actor.surname}
                    <a onClick={() => {
                        props.onDeleteActor(actor.id);
                        history.push("/shows/add");
                    }} className="btn btn-danger btn-sm" style={{marginLeft: '15px'}}><i className="fa fa-trash"
    ...
    const onFormSubmitShow = (e) => {
        ...
        formDatas.append('formData', e.target.image.files[0]);


        axios.post("http://localhost:8080/shows", newShow, {
            headers: {
                'Content-Type': "application/json"
            }
        }).then((resp) => {
            props.slika(formDatas, resp.data.id);
        });
        ...
        <div>
            <div className="bcg" style={{padding: '6%'}}>
                ...
                <form className="form-group" onSubmit={onFormSubmitShow}>
                    ...
                    <input type="text" name={"title"} className="form-control" id="showTitle" placeholder="Enter Show title here..." required/>
                    ...
                    <textarea placeholder="Enter Show description here..." className="form-control" name={"description"} id="showDescription" rows="3"
                              style={{maxHeight: '150px', minHeight:'100px', overflowY: 'scroll'}} required></textarea>
                    <div className="row">
                        <div className="col-12 col-lg-6">
                            <label htmlFor="showDirector">Director:</label>
                            <input type="text" name={"director"} className="form-control" id="showDirector"
                                   placeholder="Enter Show Director here..." required/>
                        </div>
                    ...
                    <div className="row mt-2">
                        <div className="col-sm-6 col-12 mb-3">
                            ...
                            <input type="date" className="form-control" min={year + "-" + month + "-" + day}
                                   name={"date"} id="showFrom" required/>
                        </div>
                        ...
                        <div className="col-lg-3 col-6">
                            ...
                            <input type="number" className="form-control numberInput" name={"duration"} min="0"
                                   id="showDuration" placeholder="Enter Show duration..." required/>
                        </div>
                    </div>
                    <label htmlFor="showImage">Show image:</label>
                    <input type="file" name={"image"} className="form-control-file mb-5" id="showImage" required/>
